...
- Integrated `CA\Log` (via `CaLog` facade) for structured logging across all key lifecycle operations.
- Key generation events are logged with algorithm and key size via `CaLog::keyGenerated()`.
- Key import operations are logged with format and fingerprint context.
- All errors in key generation, import, export, rotation, destruction, and decryption are caught and logged via `CaLog::critical()` before re-throwing.
